...
//      per-sample timing even when macOS batches 2–3 samples per connection
//      event
...
// ── Flash write buffer
// ──────────────────────────────────────────────────────── Buffer ~1 second of
// IMU data in RAM before flushing to flash. Keeps flash writes off the hot
// sample path and reduces wear. At 100Hz with 22-byte records: ~2200 bytes/sec
// → flush ~every 1024 bytes.
...
// ───────────── 9 bytes, packed. Replaces the old ASCII
// "timestamp,toe,heel\r\n" string.
//
// Why binary?
//   ASCII "12345,646,653\r\n" = ~17 bytes → often fragments across 2 BLE
//   packets Binary struct             =   9 bytes → always fits in a single BLE
//   packet
//
// Byte layout:
...
// ─────────────────────────────────────────────────────────────
void flushRamBuffer() {
  if (ramBufferUsed == 0 || !logFile)
    return;
  logFile.write(ramBuffer, ramBufferUsed);
  logFile.flush(); // Ensure LittleFS commits the data and updates file size!
  ramBufferUsed = 0;
}
...
void onConnect(uint16_t conn_handle) {
  ...
  // Append to previous session log instead of wiping it.
  // This prevents accidental data loss if the BLE connection drops and
  // reconnects, or if the device connects to BLE while plugged in to USB before
  // downloading. The user must now explicitly send the "CLEAR" command over USB
  // Serial to wipe.
  flushRamBuffer();
  if (logFile)
    logFile.close();
  // In Adafruit_LittleFS, FILE_O_WRITE opens in append mode (LFS_O_RDWR |
  // LFS_O_CREAT + LFS_SEEK_END)
  logFile.open(LOG_FILENAME, Adafruit_LittleFS_Namespace::FILE_O_WRITE);
  logging = logFile.operator bool();
  // Do not reset samplesWritten here so the sequence counter keeps rolling.
...
void streamDump() {
  ...
  if (!InternalFS.exists(LOG_FILENAME)) {
    Serial.println("NO_DATA: File does not exist (did you CLEAR it without "
                   "recording new data?)");
    Serial.println("EOF");
    return;
  }
  ...
  if (!reader.open(LOG_FILENAME, Adafruit_LittleFS_Namespace::FILE_O_READ)) {
    Serial.println("NO_DATA: Failed to open file for reading");
    Serial.println("EOF");
    return;
  }

  if (reader.size() == 0) {
    Serial.println("NO_DATA: File is empty (0 bytes recorded)");
  }

  Sample s;
  char line[120];
  while (reader.read((uint8_t *)&s, sizeof(s)) == sizeof(s)) {
    snprintf(line, sizeof(line), "%lu,%.3f,%.3f,%.3f,%.1f,%.1f,%.1f,%u,%u,%u",
             (unsigned long)s.device_ms, s.ax / 1000.0f, s.ay / 1000.0f,
             s.az / 1000.0f, s.gx / 10.0f, s.gy / 10.0f, s.gz / 10.0f, s.toe,
             s.heel, s.phase);
    Serial.println(line);
  }
  reader.close();
...
void handleSerialCommand() {
  ...
  if (cmd == "INFO") {
    ...
    Serial.println(1000 / SAMPLE_INTERVAL_MS);
    Serial.print("logging=");
    Serial.println(logging ? "yes" : "no");
    Serial.print("samples=");
    Serial.println(samplesWritten);
